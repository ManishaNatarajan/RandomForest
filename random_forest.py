...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        random_indexes = np.random.random_integers(0, len(XX) -1, size=n)
        XX = np.array(XX)
        samples = XX[random_indexes,:-1]
        labels = XX[random_indexes,-1]
    ...
    def fitting(self):
        ...
        for cur_tree_idx in range(self.num_trees):
            train_data = self.bootstraps_datasets[cur_tree_idx]
            label_data = self.bootstraps_labels[cur_tree_idx]
            self.decision_trees[cur_tree_idx].learn(np.array(train_data), np.array(label_data).reshape(-1,1))
        pass      
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                #predicting the most common label among the labels from the bootstrap dataset for the record 
                labs = []
                for i in range(len(self.bootstraps_datasets)):
                    dataset = self.bootstraps_datasets[i]
                    ...
                    if record in dataset:
                        ...
                        labs.append(self.bootstraps_labels[i][index])

                counts_zero = np.bincount(labs)
                y = np.append(y, np.argmax(counts_zero))
                pass
            else:
